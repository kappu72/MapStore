 ...
 */
gxp.plugins.GcSegForm = Ext.extend(Ext.FormPanel, {
    ...
    initComponent : function() {
        ...
        console.log(this.featureManager);  

        gxp.plugins.GcSegForm.superclass.initComponent.call(this);

 
 if(this.featureManager)this.featureManager.on('layerchange',function( tool, layer , schema){
    ...
    this.feature=feat;

        {
            this.items.each(function(r) {
                ...
                var name=r.getName().toUpperCase();
              
                if (this.feature.attributes[name] !== undefined) {
                    val=this.feature.attributes[name];
                  
                }
                if (val && r.xtype == "checkbox") {
                    r.setVal( Ext.isBoolean(val) ? val : (val === "true"));
                }
                else if (val && r.xtype == "gxp_datefield") {
                    r.setVal( new Date(fieldCfg.value*1000));
                }else
                if (val && r.xtype == "datefield") {
                    var dateFormat = "Y-m-d";
                    
                   r.setValue (Date.parseDate(fieldCfg.value.replace(/Z$/, ""), dateFormat));
                }else
                if (r.xtype === "combo") {
...
Ext.override(gxp.plugins.GcSegForm, gxp.plugins.SchemaAnnotations);

Ext.preg(gxp.plugins.GcSegForm.prototype.ptype, gxp.plugins.GcSegForm);
Ext.reg(gxp.plugins.GcSegForm.prototype.xtype, gxp.plugins.GcSegForm);